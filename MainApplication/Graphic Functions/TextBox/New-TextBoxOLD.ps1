...
.EXAMPLE
    New-TextBox -Parent $MyGroupBox -Type Input -TextColor 'Green' -Location (100,75) -Size (50,50)
.INPUTS
...
function New-TextBoxOLD {
    ...
    param (
        ...
        # The type of textbox (input or output). This will influence the background color, and the readonly property.
    ...
    begin {
        ...
        Add-Member -InputObject $Local:MainObject -MemberType ScriptMethod -Name Process -Value {
            ...
            $this.SetFont()
            # Add all properties to the textbox
        ...
        # Add the AddPropertyNameToTextBox method
        Add-Member -InputObject $Local:MainObject -MemberType ScriptMethod -Name AddPropertyNameToTextBox -Value {
            # Add the propertyname
            if ($this.PropertyName) {
                $this.TextBox | Add-Member -MemberType NoteProperty -Name PropertyName -Value $this.PropertyName
                # Make it interact with the registry
                $this.MakeTextBoxInteractWithRegistry($this.TextBox)
            }
        }
    
        # Add the MakeTextBoxInteractWithRegistry method
        Add-Member -InputObject $Local:MainObject -MemberType ScriptMethod -Name MakeTextBoxInteractWithRegistry -Value { param([System.Windows.Forms.TextBox[]]$Textboxes)
            # Make it interact with the registry
            $Textboxes | ForEach-Object {
                $_.Text = Invoke-RegistrySettings -Read -PropertyName $_.PropertyName
                $_.Add_TextChanged([System.EventHandler]{ param($TextBox=$_) Invoke-RegistrySettings -Write -PropertyName $TextBox.PropertyName -PropertyValue $TextBox.Text })
            }
        }
        ...
        Add-Member -InputObject $Local:MainObject -MemberType ScriptMethod -Name AddAllProperties -Value {
            ...
            $this.Properties.GetEnumerator() | ForEach-Object {
                ...
                $this.TextBox.($_.Name) = $_.Value
            }
            # If a PropertyName was passed thru, then add it
            if ($this.PropertyName) { $this.AddPropertyNameToTextBox() }
            # Add the new textbox to the parent
            $this.ParentGroupbox.Controls.Add($this.TextBox)
        }
