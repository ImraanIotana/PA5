...
function Import-FeatureOmnissaDEMUserRegistry {
    [CmdletBinding()]
    param (
    ...
    begin {
        ...
        [PSCustomObject]$GroupBox   = @{
            Title                   = [System.String]'Omnissa DEM user REGISTRY'
            Color                   = [System.String]'Blue'
            NumberOfRows            = [System.Int32]2
            GroupBoxAbove           = $Global:OmissaDEMUserFilesGroupbox
        }
        ...
        [System.Collections.Hashtable[]]$ActionButtons = @(
            @{
                ColumnNumber    = 1
                Text            = 'IMPORT UserRegistry Objects (*)'
                Image           = 'table_add.png'
                SizeType        = 'Large'
            ...
            @{
                ColumnNumber    = 2
                Text            = 'EXPORT UserRegistry Objects'
                Image           = 'table_go.png'
                SizeType        = 'Large'
                ToolTip         = 'Export the User Registry Objects of this application from Omnissa DEM to the DSL'
                Function        = { Export-OmnissaDEMObjects -ApplicationID $Global:ODMApplicationIDComboBox.Text -UserRegistry }
            }
            @{
                ColumnNumber    = 3
                Text            = 'Open UserRegistry Repository'
                Image           = 'folder_table.png'
                SizeType        = 'Large'
                ToolTip         = 'Open the Repository containing the objects'
                Function        = { Open-Folder -Path $Global:OmissaDEMUserRegistryGroupbox.CorrespondingRepository }
            }
            @{
                ColumnNumber    = 5
                Text            = 'REMOVE UserRegistry Objects (*)'
